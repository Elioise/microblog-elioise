...
//Se não houver um id válido na URL, faça voltar pra a pagina usuarios
...
try {
	...
	if(!$dados) $erro = "Usuario não encontrado";
} catch (\Throwable $e) {
	$erro = "Erro ao buscar usúario. <br>" .$e->getMessage();}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if (empty($_POST['nome'])|| empty($_POST['email'])||empty($_POST['tipo'])) {
        $erro = "Nome, e-mail e tipo são obrigatorios";
    } else {
